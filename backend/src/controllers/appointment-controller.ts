...
import { isFuture, asDate } from "../utils/time";
import { HttpError } from "../errors/HttpError";

const QUERIES_IN_THE_PAST = "Já existe uma consulta para este médico neste horário.";
...
export const createAppointment = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (existing) {
      return next(new HttpError(400, QUERIES_IN_THE_PAST));
    }
